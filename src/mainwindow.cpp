...
void MainWindow::on_load_library(QString path)
{
    ...
    if(!SettingsHandler::getHideStandAloneFunscriptsInLibrary())
    {
...
void MainWindow::on_playVideo(LibraryListItem selectedFileListItem, QString customScript, bool audioSync)
{
    ...
    if (file.exists())
    {
        if ((selectedFileListItem.type == LibraryListItemType::FunscriptType && _syncHandler->getPlayingStandAloneScript() != selectedFileListItem.path) ||
                (videoHandler->file() != selectedFileListItem.path || !customScript.isEmpty()))
        {
            QString scriptFile;
...
void MainWindow::on_PlayBtn_clicked()
{
    if (libraryList->count() > 0)
    {
        if(libraryList->selectedItems().length() == 0)
        {
            ...
            stopAndPlayMedia(selectedItem->getLibraryListItem());
            return;
        }
        ...
        if(selectedFileListItem.path != videoHandler->file() || (!videoHandler->isPlaying() && !_syncHandler->isPlayingStandAlone()))
        {
...
//        LogHandler::Debug("VR playing: "+QString::number(packet.playing));
    ...
    {
        LogHandler::Debug("onVRMessageRecieved video changed: "+packet.path);
        LogHandler::Debug("onVRMessageRecieved old path: "+vrScriptSelectedCanceledPath);
    ...
    if (!vrScriptSelectorCanceled && !packet.path.isEmpty() && !funscriptFileSelectorOpen && packet.duration > 0)
    {
        ...
        if (funscriptPath.isEmpty() || !funscriptFile.exists())
        {
            ...
            else
            {
                ...
                LogHandler::Debug("onVRMessageRecieved funscript not found in path of media");
                LogHandler::Debug("onVRMessageRecieved Searching in library root...");
