#include "audiosyncfilter.h"

AudioSyncFilter::AudioSyncFilter(QObject *parent)
    : AudioFilter(parent)
    , mLeft(0)
    , mRight(0)
{
...
void AudioSyncFilter::process(Statistics *statistics, AudioFrame *frame)
{
    if (!frame)
        return;
    ...
    if (!qFuzzyCompare(level[0], mLeft) || !qFuzzyCompare(level[1], mRight))
